...
import { updateExamRoutine } from "@/server/actions/examRoutine/examRoutine.action";

const EditExamRoutine = ({ routine, id }: { routine: any; id: string }) => {
  const {
    handleSubmit,
    register,
    watch,
  ...
  useEffect(() => {
    setValue("grade", routine.grade);
    trigger("grade");
    setValue("exam", routine.exam);
    trigger("exam");
  }, []);
  ...
  const onSubmit = async (data: TExamRoutineForm) => {
    ...
    const res = await updateExamRoutine(id, data);
    toast({
