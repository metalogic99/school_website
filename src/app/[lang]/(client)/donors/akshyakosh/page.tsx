...
export default async function page({ params }: { params: { lang: string } }) {
  const { lang } = params;
  const title = lang === "en" ? "Akshyakosh Donors" : "अक्षयकोष दाताहरू";

  const donors = await Donors.find({ donation_type: "akshyakosh" });
  ...
  return (
    <div>
      ...
      <section className=" mx-auto mt-8 flex flex-col gap-8 py-20">
        <div className=" mx-auto flex  max-w-7xl flex-wrap items-center justify-center gap-8">
          {donors &&
            donors.length > 0 &&
            donors.map((pp: any, idx: number) => {
              return (
                <div key={idx} className="flex  w-[270px] flex-col text-wrap  ">
                  <img
