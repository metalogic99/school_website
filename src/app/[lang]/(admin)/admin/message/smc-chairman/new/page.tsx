...
import { addSmcChairmanMessage } from "@/server/actions/messages/smc.chairman.messages.action";
...
const page = () => {
  ...
  } = useForm<TSmcChairmanMessageForm>({
    resolver: zodResolver(SmcChairmanMesssageSchema),
  });
  ...
  const onSubmit = async (data: TSmcChairmanMessageForm) => {
    const imgRes = await handleImageUpload();
    const res = await addSmcChairmanMessage(data, imgRes);
    toast({
