...
import { addMessage } from "@/server/actions/messages/message.action";
...
const page = () => {
  ...
  const onSubmit = async (data: MessageInput) => {
    const imgRes = await CloudinaryUpload({ image, folder: "message" });
    const res = await addMessage(data, imgRes);
    toast({
